...
First, suppose we select the top 3% of the distribution. [(This comprises the values between the $0.97$ and $1$ -quantiles of this distribution.)](https://en.wikipedia.org/wiki/Quantile)
...
**1. Derivation of the pdf $f_Y(Y)$**[^1]: A "proof" that the pdf of $Y = F_X(X)$ is $1$, starting from the insight given above.

Let $a, b$ be real numbers such that $0 \leq a < b \leq 1$.

By the argument above, we have $F_Y(b) - F_Y(a) = b - a$.

(We will rewrite this statement as $F(b) = b \text{ and } F(a) = a$, i.e. $F$ is the identity.)
...
**2. Proof**: the standard proof of the PIT found on the [Wikipedia page](https://en.wikipedia.org/wiki/Probability_integral_transform).
