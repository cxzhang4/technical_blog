...
The probability integral transform states that, for a continuous random variable $X$, the distribution of $Y = F_X(X)$ is $U(0, 1)$. This result underlies inverse transform sampling. It is also central to why p-values are uniformly distributed under the null hypothesis. But why does this make sense?
...
**Proof**: the standard proof of the PIT.
...
And since $F_{T; H_0}(T)$ is a random variable's CDF applied to itself, its distribution is standard uniform by the probability integral transform. Therefore, $1 - F_{T; H_0}(T) \sim U(0, 1)$, and we have shown that p-values are uniformly distributed under the null hypothesis.

## Appendix

[^1]: This is not necessary: once we know $F_Y(Y)$, we know the distribution of $Y$. I still found it instructive to work through these steps.