...
description: "The probability integral transform states that, for a continuous random variable $X$, the distribution of $Y = F_X(X)$ is $U(0, 1)$. I discuss why this makes sense."
author: "Carson Zhang"
date: "12/04/2023"
draft: false
---

The probability integral transform states that, for a continuous random variable $X$, the distribution of $Y = F_X(X)$ is $U(0, 1)$. This result underlies inverse transform sampling. I suspect that it's also central to why p-values are normally distributed under the null hypothesis. But why does this make sense?
...
It is natural for me to suspect the density of curve of the CDF-transformed sample to be a horizontal line at $1$: this is the only function I can think of that guarantees this property.

We define the following lemma and "proof" that a random variable's CDF applied to itself has a standard uniform distribution.
...
And we know $1 - Y \sim U(0, 1)$, so $P_{H_0}(T \geq t(x)) \sim U(0, 1)$.
