...
\begin{align}
F_P(P) &= F_{T; H_0}(T)\\
  &= F_{U(0, 1)} && \text{(probability integral transform)}
\end{align}
$$
Since $F_{T; H_0}(T)$ is a random variable's CDF applied to itself, its distribution is standard uniform by the probability integral transform.
...
Therefore, we have shown that one-sided p-values are uniformly distributed under the null hypothesis.[^2]
...
[^2]: [This holds only for divergence p-values, not decision p-values](https://arxiv.org/abs/2301.02478). My understanding is that divergence p-values are exactly one-sided p-values. Thanks again to Prof. Schomaker for this insight, and for bringing this to my attention.